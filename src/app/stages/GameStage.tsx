...
import { chance, last, withLast } from "../util";
import { Link } from "../components/Link";
import { GameConnection, EMOJI_TIMER_SECS } from "../game";
...
export default function GameStage({ game, visible, matchState }: {
  ...
  const [partnerPickedBefore, setPartnerPickedBefore] = useState(false);
  const [wasSearching, setWasSearching] = useState(true);
  const [endSfxPlayed, setEndSfxPlayed] = useState(false);

  const [lastTimeLeft, setLastTimeLeft] = useState(-1);

  const [sndClick] = useSound("/audio/click.wav");
  const [sndGameLost] = useSound("/audio/game_lost.wav");
  const [sndGameWinSpecial] = useSound("/audio/game_win_special.wav");
  const [sndGameWin] = useSound("/audio/game_win.wav");
  const [sndPartnerSelect] = useSound("/audio/partner_select.wav");
  const [sndWarnCountdown] = useSound("/audio/warn_countdown.wav");
  const [sndGameStart] = useSound("/audio/game_start.wav");

  const timeLeft = Math.max(0, EMOJI_TIMER_SECS - Math.floor((new Date().getTime() - lastPickedAt.getTime()) / 1000));
  ...
  useEffect(() => {
    if (matchState !== null && matchState.state === "ENDED" && !endSfxPlayed) {
      if (matchState.history.length != 0 && withLast(matchState.history, x => x.partner == x.you)) {
        if (chance(1 / 3)) {
          // YIPPPEEEEEEE!!
          sndGameWinSpecial();
        } else {
    ...
    if (!partnerPickedBefore && matchState.partnerPicked) {
      sndPartnerSelect();
      setPartnerPickedBefore(true);
    }

    if (timeLeft != lastTimeLeft) {
      setLastTimeLeft(timeLeft);

      if (timeLeft <= 3 && lastTimeLeft != -1) {
        sndWarnCountdown();
      }
    }
    ...
    if (prevHistoryLength != matchState.history.length) {
      ...
      if (!partnerPickedBefore) {
        sndPartnerSelect();
      }

      setPartnerPickedBefore(false);
    }
  }, [matchState]);
  ...
  return (
    <section className={`
      ...
      ${matchState?.state === "MATCHMAKING" ? "md:justify-center" : ""}
      flex flex-col p-8 py-0 w-full h-full transition-opacity duration-500 absolute top-0 left-0`
    }>
      {
        matchState === null || game === null || matchState.state === "MATCHMAKING"
          ...
          : matchState.state === "ONGOING" || matchState.state === "ENDED"
            ...
            : <div className="md:px-64 my-auto">
              <audio className="hidden" src="/audio/opponent_left.wav" autoPlay />
              <main className={`
                flex flex-col gap-4 w-full text-center justify-center
                bg-[#ffe2b6] rounded-lg px-16 py-16 drop-shadow-sm
              `}>
                <div className="text-3xl">{emojiAnimationFrames[emojiAnimFrame]}</div>
      ...
      <div className={`
        ${matchState?.state !== "ENDED" ? "opacity-0 pointer-events-none" : "opacity-100"}
        transition-opacity fixed left-16 bottom-16 bg-[#fffde4] p-8 rounded-lg
      `}>
